...
function renderCasinoTables(tables) {
  ...
  const nr        = tables.filter(t => t.game === 'neon_racer');
  ...
  if (el('casino-db-tables'))       el('casino-db-tables').innerHTML       = db.map(t=>renderTableCard(t)).join('');
  if (el('casino-nr-tables'))       el('casino-nr-tables').innerHTML       = nr.map(t=>renderTableCard(t)).join('');
...
function renderTableCard(t) {
  ...
  const gameIcons = { poker:'🃏', blackjack:'🃏', slots:'🎰', roulette:'🎡', pachinko:'🎯', crash:'🚀', coinflip:'🪙', path_of_gambling:'⚗️', jackpot_frenzy:'🏆', dragon_hoard:'🐉', arcane_academy:'🔮', dual_blades:'⚔️', neon_racer:'🏎️' };
  const gameColors = { poker:'', blackjack:'bj', slots:'slots', roulette:'roulette', pachinko:'pachinko', crash:'crash', coinflip:'coinflip', path_of_gambling:'path', jackpot_frenzy:'slots', dragon_hoard:'dh', arcane_academy:'aa', dual_blades:'db', neon_racer:'nr' };
...
async function openCasinoTable(tableId) {
  ...
  if (game === 'neon_racer')     initNRUI(table);
